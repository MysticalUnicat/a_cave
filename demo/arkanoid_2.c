...
static void _playing_frame(void * ud) {
  ...
  alias_Physics2DLinearMotion_write(g.paddle)->velocity.x = (IsKeyDown(KEY_RIGHT) ? 500 : 0) - (IsKeyDown(KEY_LEFT) ? 500 : 0);
...
static void _start_begin(void * ud) {
  ...
  g.ball = SPAWN(
                 ...
                 , ( alias_Physics2DLinearMotion, .damping = 0.995f )
                 , ( alias_Physics2DLinearMass, .mass = 1.0f )
...
int main(void) {
  ...
  while(!WindowShouldClose()) {
    ...
    physics_transform_update(g.physics_speed);
